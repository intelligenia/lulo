...
/**
* Fichero para la clase DB
* @package helper
* @author intelligenia
*/

/**
* Clase DB. Da funciones útiles para el trabajo con bases de datos
*
* @package helper
*/
class DB
  ...
  *			'Comment' => string comentarios sobre la columna en el índice
	...
	*/
	public static function convertDataArrayToSQLModificationString($new_data)
	{
		...
		{
			if(is_string($nvalue)) $nvalue = static::$db_connection->qstr($nvalue);
			elseif(is_null($nvalue)) $nvalue = "NULL";
			elseif(is_bool($nvalue)) $nvalue = $nvalue?1:0;
			elseif(is_float($nvalue)) $nvalue = str_replace(",",".",$nvalue);
		}
	...
	*/
	public static function convertDataArrayToSQLUpdateString($new_data)
	{
		...
		foreach($new_data as $nkey=>$nvalue)
		{
			if(is_null($nvalue)) $new_data_sql .= "$nkey=NULL";
			elseif(is_bool($nvalue)) $new_data_sql .= "$nkey=".($nvalue?"1":"0");
			elseif(is_float($nvalue)) $new_data_sql .= "$nkey=".str_replace(",",".",$nvalue);
			else $new_data_sql .= $nkey.DB::strEq($nvalue);
			$new_data_sql .= ",";
		}
	...
	*/
	...
	{
		if($extraCondition==null) $extraCondition = array();
	...
	*/
	...
	{
	   foreach($tables as $table)
		if(!self::existsTableInCurrentDatabase(trim($table))) return false;
	   return true;
	}
	...
	*/
	public static function getCreateTable($tableName)
	{
		...
		if(!isset($res["Create Table"]) or !is_string($res["Create Table"])) return "";
